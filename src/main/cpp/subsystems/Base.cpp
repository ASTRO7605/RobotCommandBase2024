...
Base::Base()
    : m_PoseEstimator{kDriveKinematics,
                      ...
                      PoseEstimationConstant::
                          kVisionStdDevs}, // important to follow kinematics
                                           // construction order
      m_VisionFront{VisionConstant::TableNameFront},
      m_VisionBack{VisionConstant::TableNameBack} {
  ...
  */
  ...
  pathplanner::AutoBuilder::configureHolonomic(
      ...
      [this](frc::ChassisSpeeds speeds) {
        DriveRobotRelativeChassisSpeeds(speeds);
      },
      pathplanner::HolonomicPathFollowerConfig(
          pathplanner::PIDConstants(
              DriveConstant::kPAutoMovementController, // P
              0.0,                                     // I
              0.0                                      // D
              ),                                       // PID translation
          ...
          pathplanner::ReplanningConfig(
              true, true) // can be tweaked for further control
          ),
      []() {
...
void Base::Periodic() {
  ...
  frc::SmartDashboard::PutNumber("Gyro Radians",
                                 m_Gyro.GetRotation2d().Radians().value());

  frc::SmartDashboard::PutNumber(
      "RobotPoseX", m_PoseEstimator.GetEstimatedPosition().X().value());

  frc::SmartDashboard::PutNumber(
      "RobotPoseY", m_PoseEstimator.GetEstimatedPosition().Y().value());
...
void Base::Drive(units::meters_per_second_t xSpeed,
  ...
  if (rateLimiting) {
    ...
    if (angleDif < 0.45 * std::numbers::pi) {
      ...
      if (m_CurrentTranslationMag >
          1e-4) { // some small number to avoid floating-point
  ...
  units::meters_per_second_t xSpeedDelivered =
      xSpeedCommanded * DriveConstant::kMaxTeleopSpeed;
  units::meters_per_second_t ySpeedDelivered =
      ySpeedCommanded * DriveConstant::kMaxTeleopSpeed;
...
units::angle::degree_t Base::GetHeadingDegrees() {
...
frc::ChassisSpeeds Base::GetRobotRelativeSpeeds() {
  return kDriveKinematics.ToChassisSpeeds(
      m_FrontRightModule.GetState(), m_FrontLeftModule.GetState(),
...
void Base::SetRobotPoseVisionEstimateFront() {
  ...
  auto std_devs =
      PoseEstimationConstant::kVisionStdDevs_XYPerMeterSquared_Front;
  auto dst_sq = estimate.distance.value() * estimate.distance.value();
  std_devs[0] *= dst_sq; // scale based on distance
  std_devs[1] *= dst_sq;

  frc::SmartDashboard::PutNumber("april_distance_front",
                                 estimate.distance.value());
...
void Base::SetRobotPoseVisionEstimateBack() {
  ...
  frc::SmartDashboard::PutNumber("april_distance_back",
                                 estimate.distance.value());
...
void Base::SwitchRobotDrivingMode() {
  m_DrivingInFieldRelative = !m_DrivingInFieldRelative;
}

void Base::ResetGyroTeleopOffset() {
  m_GyroOffset = m_Gyro.GetRotation2d().Radians();
}