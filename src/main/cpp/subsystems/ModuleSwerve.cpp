...
ModuleSwerve::ModuleSwerve(int TurningMotorID, int DrivingMotorID, int CANcoderID)
 : m_TurningMotor{TurningMotorID, rev::CANSparkMax::MotorType::kBrushless},
   ...
   m_TurningSparkMaxEncoder {m_TurningMotor.GetEncoder(rev::SparkRelativeEncoder::Type::kHallSensor)},
   ...
   m_DesiredState {units::meters_per_second_t{0.0}, frc::Rotation2d()} 
   {
    ...
    m_DrivingEncoder.SetPositionConversionFactor(DriveConstant::kWheelCirconfM / DriveConstant::kDrivingGearRatio); //from motor rotations to meters *to test
    m_DrivingEncoder.SetVelocityConversionFactor((DriveConstant::kWheelCirconfM / DriveConstant::kDrivingGearRatio) / 60); //m/s
    m_DrivingEncoder.SetPosition(0);

    m_TurningSparkMaxEncoder.SetPositionConversionFactor((2 * std::numbers::pi) / DriveConstant::kTurningGearRatio); //from motor rotations to radians
    m_TurningSparkMaxEncoder.SetVelocityConversionFactor(((2 * std::numbers::pi) / DriveConstant::kTurningGearRatio) / 60); //radians/s
    m_TurningSparkMaxEncoder.SetPosition(units::radian_t{m_TurningCANcoder.GetAbsolutePosition().GetValue()}.value()); //seed les spark max avec la valeur du CANcoder

    m_TurningPIDController.SetPositionPIDWrappingEnabled(true); //PID controller can go through 0 to get to setpoint
    ...
    m_DesiredState.angle = frc::Rotation2d(units::radian_t{m_TurningCANcoder.GetAbsolutePosition().GetValue()}); //on dit aux swerves de garder leur position initiale
}

frc::SwerveModuleState ModuleSwerve::GetState() {
  return {units::meters_per_second_t{m_DrivingEncoder.GetVelocity()}, units::radian_t{m_TurningSparkMaxEncoder.GetPosition()}};
}

frc::SwerveModulePosition ModuleSwerve::GetPosition(){
  return {units::meter_t{m_DrivingEncoder.GetPosition()}, units::radian_t{m_TurningSparkMaxEncoder.GetPosition()}};
}

void ModuleSwerve::SetDesiredState(frc::SwerveModuleState desiredState){

  frc::SwerveModuleState optimizedDesiredState {frc::SwerveModuleState::Optimize( //permet aux roues de ne jamais aller plus de 90 degrés à la fois
    desiredState, frc::Rotation2d{units::radian_t{m_TurningSparkMaxEncoder.GetPosition()}}
  )};

  m_DrivingPIDController.SetReference(optimizedDesiredState.speed.value(), rev::CANSparkMax::ControlType::kVelocity);
...
void ModuleSwerve::ResetEncoders(){
  m_DrivingEncoder.SetPosition(0);
}
